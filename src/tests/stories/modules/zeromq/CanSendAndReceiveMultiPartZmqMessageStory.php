...
$story->addAction(function() {
	...
	foreach(firstHostWithRole("zmq_target") as $hostId) {
		...
		$outPort = fromHost($hostId)->getAppSetting("zmq.multi.outPort");
		...
		usingZmqSocket($inSocket)->sendMulti($checkpoint->expectedMessage, 1);
		$checkpoint->actualMessage = fromZmqSocket($outSocket)->recvMulti(1);
	}
});
